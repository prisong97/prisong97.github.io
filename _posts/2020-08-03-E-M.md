...
For $(4)$ to hold with equality, it is sufficient to set $Q(z^{(i)}_j) = f(z_i \mid x^{(i)}, \theta)$ [a], the posterior probability of picking class $z_j$ given that we have observed the $ith$ observation $x^{(i)}$.
